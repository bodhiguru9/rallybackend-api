...
async function findDuplicateJoins() {
  try {
    ...
    console.log(`✅ Found ${allEvents.length} events. Now fetching participants for each event...`);

    const duplicateEvidence = [];

    // Check participants for each event
    for (const event of allEvents) {
      // We can only fetch participants for public events without authentication
      ...
      while (pHasMore) {
        try {
          ...
          if (pResponse.data && pResponse.data.success) {
            const currentParticipants = pResponse.data.data.participants;
            participants = participants.concat(currentParticipants);
            
            const pPagination = pResponse.data.data.pagination;
            if (!pPagination.hasNextPage) {
              pHasMore = false;
            } else {
              pPage++;
            }
          ...
          // Ignore errors for individual events (might be deleted or restricted)
          pHasMore = false;
        }
      }
    ...
    if (duplicateEvidence.length > 0) {
      ...
      console.log('No duplicates found in public events. (Private events could not be checked without authentication).');
    }
